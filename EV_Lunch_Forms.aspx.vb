...
Public Class EV_Lunch_Forms
	...
	Sub Letter()
		...
		Dim con As New SqlConnection
		Dim cmd As New SqlCommand
		Dim dayDateSQL As String = "SELECT id, CONCAT(SUBSTRING(DATENAME(WEEKDAY, visitDate), 1, 3), ' - ', FORMAT(visitDate, 'M/d/yy')) as DateAndDay
									, (CASE
									 ...
									 END) AS pickUpTime
									 FROM visitInfo
									 WHERE DATENAME(MONTH, visitDate) = '" & month_ddl.SelectedValue & "'
									 AND YEAR(visitDate) = '" & year_ddl.SelectedValue & "'
									 AND NOT DATENAME(WEEKDAY, visitDate)='Saturday' AND NOT DATENAME(WEEKDAY, visitDate)='Sunday'
									 ORDER BY visitDate"
