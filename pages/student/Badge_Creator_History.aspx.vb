...
Public Class Badge_Creator_History
    ...
    Dim BadgeData As New Class_BadgesData
    ...
    Sub LoadBadgesIntoGridview()
        ...
        Dim SQLOrderBy As String = " ORDER BY id DESC"
        ...
        If studentSearch_ddl.SelectedIndex <> 0 Then
            ...
            SQLWhereSearch = " AND (employeeNumber = '" & StudentAccountNum(0) & "')"
        End If

        'Load badges into table
        Try
            existingBadges_dgv.DataSource = BadgeData.LoadExistingBadgesTable(Visit, SQLWhereSearch, SQLOrderBy)
            existingBadges_dgv.DataBind()
        Catch
    ...
    Sub LoadStudentNamesDDL()
        ...
        Try
            BadgeData.LoadExistingBadgesNamesDDL(Visit, studentSearch_ddl)
    ...
    Private Sub existingBadges_dgv_RowDeleting(sender As Object, e As GridViewDeleteEventArgs) Handles existingBadges_dgv.RowDeleting
        ...
        Try
            ...
            con.Open()
            cmd.Connection = con
            cmd.CommandText = "SELECT employeeNumber, employeeName FROM badges WHERE id='" & ID & "'"
            dr = cmd.ExecuteReader

            While dr.Read()

                If dr("employeeName").ToString <> Nothing Then
                    deletingName_hf.Value = dr("employeeName").ToString
                Else
                    deletingName_hf.Value = dr("employeeNumber").ToString
                End If
        ...
        deletingName_lbl.Text = deletingName_hf.Value
